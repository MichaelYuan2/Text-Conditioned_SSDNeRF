...
class TextGaussianDiffusion(nn.Module):
    ...
    def __init__(self,
        ...
        self.img_size = code_size[-1]
        self.text_embedding_size = text_embedding_size
        self.text_cond_mlp_down = nn.Linear(self.text_embedding_size, mlp_hidden)
        self.text_cond_mlp_up = nn.Linear(mlp_hidden, self.img_size * self.img_size)
        self.relu = nn.ReLU()
    ...
    def pred_x_0(self, x_t, t, grad_guide_fn=None, concat_cond=None, text_cond=None, cfg=dict(), update_denoising_output=False):
        ...
        if text_cond is not None:
            text_cond = self.text_cond_mlp_down(text_cond)
            text_cond = self.text_cond_mlp_up(text_cond)
            text_cond = self.relu(text_cond)
            text_cond = text_cond.reshape(-1, 1, self.img_size, self.img_size)
